<?php  
	$id_card = $_GET["editCard"];
	$card_info = array();
	$card_info = get_info_card_ById($id_card);
?>
			...
			<div class="row">
				<div class="col-md-6">
					<div id="grp_name" class="input-group">  
						...
						<input type="text" id="txt_num" class="form-control" aria-describedby="basic-addon1" value="<?php echo $card_info[1]; ?>">
					</div>
					<div id="grp_last" class="input-group">  
						<label class="control-label" for="txt_name">Nombre en Tarjeta</label>
						<input type="text" id="txt_name" class="form-control" aria-describedby="basic-addon2" value="<?php echo $card_info[2]; ?>" >
					</div>
					
					<div id="grp_date" class="input-group">  
						<label class="control-label" for="month">Fecha Vencimiento </label>
						<div class="row">
							<div class="col-xs-5">
...
<script>
	...
	$("#btn_changeData").click(function(){
		...
		if(message == ""){
			ajax_("cards", "&todo=edit&numCard_="+numCard+"&nameCard_="+name+"&month_="+mes+"&year_="+año+"&id_=<?php echo $card_info[0]; ?>", false, "pruebaDB");
